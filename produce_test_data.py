...
def get_300d_features():
    '''

    :return: the path which save the ckpt file
    '''
    train_data_for_rnn_50 = {}
    data_root = r"D:\mec_data2\train_set"
    npy_root = r"D:\mec_data2" + os.sep + "train_image_counter_higher_50_npy.npy"
    if os.path.exists(npy_root) == True:
        train_image_counter_higher_50 = np.load(npy_root).tolist()
    for key in train_image_counter_higher_50:
        train_data_for_rnn_50[key] = []

    emotion = Emotion()
    h_fc1 = emotion.build_network()
        ...
        for key in train_image_counter_higher_50:
            ...
            for image_name in image_names:
                if var < 50:
                    ...
                    feature = sess.run(h_fc1,feed_dict={
                        emotion.x: img,
                        emotion.is_training: False
                    })
                    train_data_for_rnn_50[key].append(feature[0])
                    var = var + 1
                else:
                    break
            print(key, current_var, time.time() - current_time)
            current_var = current_var + 1
        np.save("./npy_data/train_data_for_rnn.npy", train_data_for_rnn_50)
